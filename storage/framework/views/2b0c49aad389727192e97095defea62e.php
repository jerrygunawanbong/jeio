...
<head>
    ...
    <title>jeio — <?php echo e($roomInfo['name']); ?></title>
...
<body class="min-h-screen flex flex-col items-center justify-between p-3 md:p-6 select-none relative overflow-x-hidden">
    ...
    <div id="setupModal" class="fixed inset-0 bg-[#283618]/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div class="bg-[#faedcd] neo-box rounded-3xl p-6 md:p-8 w-full max-w-md relative">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 class="text-3xl font-black text-[#283618] leading-none"><?php echo e($roomInfo['name']); ?></h2>
                    <p class="text-xs font-black text-[#bc6c25] uppercase tracking-wider mt-1">
                        <?php echo e($roomInfo['is_private'] ? '🔒 Private Room' : '🌐 Public Room'); ?>
    ...
    <header class="w-full max-w-6xl flex flex-col sm:flex-row items-center justify-between gap-3 mb-4 z-10">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-3xl font-black tracking-wider text-[#283618]"><?php echo e($roomInfo['name']); ?></h1>
                <p class="text-[10px] font-black text-[#bc6c25] tracking-widest uppercase">
                    Status: <span class="text-[#283618] font-mono"><?php echo e($roomInfo['is_private'] ? '🔒 Private' : '🌐 Public'); ?></span> (<span><?php echo e($roomInfo['max_players']); ?> Max</span>)
                </p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 bg-[#faedcd] border-2 border-[#283618] p-1.5 rounded-2xl w-full sm:w-auto shadow-[3px_3px_0px_#283618]">
            <input type="text" id="shareUrl" readonly value="<?php echo e(url('/room/' . $roomId)); ?>" class="bg-transparent text-xs text-[#283618] font-black font-mono px-3 py-1 outline-none w-full sm:w-60 truncate">
            ...
            <div>
                ...
                <div id="chatMessages" class="flex flex-col gap-2.5 overflow-y-auto h-[440px] pr-1 text-xs">
                    <div class="bg-[#fefae0] border-2 border-[#283618] rounded-xl p-2.5 text-[#283618] font-bold italic shadow-[2px_2px_0px_#283618]">
                        👋 Selamat datang di room <b><?php echo e($roomInfo['name']); ?></b>! Ketik pesan di bawah buat ngobrol.
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        const isPrivateRoom = <?php echo e($roomInfo['is_private'] ? 'true' : 'false'); ?>;
        let socketId = null;
        let isSyncing = false;
        ...
        <?php if(isset($savedCanvas) && $savedCanvas): ?>
            isSyncing = true;
            const img = new Image();
            img.onload = function() {
            ...
            img.src = <?php echo $savedCanvas; ?>.imgData || '';
        <?php endif; ?>

        const PUSHER_APP_KEY = "<?php echo e(env('PUSHER_APP_KEY')); ?>";
        const PUSHER_CLUSTER = "<?php echo e(env('PUSHER_APP_CLUSTER', 'ap1')); ?>";
...
</html><?php /**PATH C:\laragon\www\anonimcanvas\resources\views/canvas.blade.php ENDPATH**/ ?>