                ...
                <div class="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-1">
                    @forelse($rooms as $room)
                        <div class="bg-[#fefae0] border-2 border-[#283618] rounded-2xl p-3 flex items-center justify-between shadow-[2px_2px_0px_#283618]">
                            <div>
                                <h3 class="font-black text-sm text-[#283618]">{{ $room['name'] }}</h3>
                                <span class="text-[10px] font-bold text-[#bc6c25]">Max {{ $room['max_players'] }} Players • Jam {{ $room['created_at'] }}</span>
                            </div>
                            <a href="/room/{{ $room['id'] }}" class="bg-[#283618] text-[#fefae0] text-xs font-black px-3.5 py-2 rounded-xl neo-btn">
                                Masuk 🚀
                            </a>
                        </div>
                    @empty
